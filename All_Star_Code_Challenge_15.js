...
// You're father is far too busy with the business to worry about such details, so, naturally, he's making
// you come up with the text strings.
...
// array should be the rotated version of the previous element. The output array SHOULD be the same length as 
// the input string The function should return an emptry array with a 0 length string, '', as input

function rotate(str) {
    let mass = [];
    if (str.length === 1) { mass.push(str); return mass; }
    for (let i = 1; i <= str.length; i++) {
        mass.push(str.trim().slice(i) + str.trim().slice(0, i));
    }
    return mass;
}