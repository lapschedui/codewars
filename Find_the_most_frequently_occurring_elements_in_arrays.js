...
// Each of the 5 arrays includes 8 numbers which represent 3-hourly temperature forecast for a given day.

// Output: An array which includes the most frequent number (temperature) 
// from each of the 5 arrays (days). In case there is a tie, return the value present later in a given array (day).
...
function getMostFrequent(json) {
    let result = [];
    let m = { number: 0, quantity: 0 };
    for (let a of json.temperature) {
        for (let n of a) {
            if (a.filter(item => item === n).length >= m.quantity) {
                m.number = n;
                m.quantity = a.filter(item => item === n).length;
            }
        }
        result.push(m.number);
        m = { number: 0, quantity: 0 };
    }
    return result;
}