...
// Contrary to popular belief, fish actually reduce pond maintenance as they graze on string algae 
// and bottom feed from the pond floor. They also make very enjoyable pets, providing hours of natural entertainment.
// Task
...
// 8.  The bigger fish you eat, the faster you grow. A size 2 fish equals two size 1 fish, 
// size 3 fish equals three size 1 fish, and so on.
...
// Current size     Amount extra needed       Total size        Increase to size
//                  for next size               1 fish
// 1                     4                        4                      2
// 2                     8                        12                     3
// 3                     12                       24                     4
// 4                     16                       40                     5
...
function fish(shoal) {
    let anyFish = shoal.split('');
    let size = 1;
    let n = 0;
    for (let a = 1; a <= size; a++) {
        n += anyFish.filter(item => item == a).length * a;
        for (let m = size * 4; m <= 100; m += 4) {
            if (n >= m) {
                n -= m;
                size++;
            }
